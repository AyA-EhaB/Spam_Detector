...
def get_example_messages():
    examples = {
        ...
        "Spam Examples": [
            "URGENT: You have won a $1000 gift card. Call now to claim!",
        ...
        with st.spinner('Analyzing message...'):
            ...
            if not preprocessed:
                ...
                with col1:
                    ...
                    st.progress(lr_confidence if lr_pred else 100 - lr_confidence)
                ...
                with col2:
                    ...
                    st.markdown(f'<div class="spam-result {prediction_class}">{prediction_text}</div>', unsafe_allow_html=True)
                    st.progress(rf_confidence if rf_pred else 100 - rf_confidence)
...
with tab2:
    ...
    for category, messages in examples.items():
        ...
        for i, example in enumerate(messages):
            if st.button(f"Example {i+1}", key=f"{category}_{i}"):
                # Set the example as the active message and rerun
                st.session_state.example_message = example
                st.experimental_rerun()
    
    # Check if we need to process an example
    if 'example_message' in st.session_state and st.session_state.example_message:
        msg = st.session_state.example_message
        st.session_state.example_message = None
        # Jump to the first tab and analyze
        st.session_state.active_tab = "Check Your Message"
        st.experimental_rerun()
